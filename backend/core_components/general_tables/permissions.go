...
// Poistaa restricted.auth_group_table_func_rights -taulusta kaikki rivit,
// joiden target_table_name on voimassaolon ulkopuolella (ei löytynyt skeemasta)
// TAI joiden function_id viittaa pois käytöstä (disabled) olevaan funktioon.
func Remove_non_existent_table_rights(db *sql.DB) error {
	deleteQuery := `
        DELETE FROM restricted.auth_group_table_func_rights agr
        WHERE
            (
	...
	log.Println("Poistetaan restricted.auth_group_table_func_rights -taulusta puuttuvat tai disabloituihin funktioihin viittaavat rivit...")
	...
	log.Printf("Poistettu %d riviä restricted.auth_group_table_func_rights -taulusta", rowsAffected)
	return nil
}

// Poistaa restricted.auth_group_table_func_rights -taulusta ne rivit, joiden function_id
// puuttuu kokonaan restricted.functions-taulusta TAI on merkitty disabled = true.
func Remove_non_existent_or_disabled_functions(db *sql.DB) error {
	deleteQuery := `
		DELETE FROM restricted.auth_group_table_func_rights
		WHERE NOT EXISTS (
			SELECT 1
			FROM restricted.functions
			WHERE restricted.functions.id = restricted.auth_group_table_func_rights.function_id
			  AND restricted.functions.disabled = false
		)
	`

	log.Println("Poistetaan restricted.auth_group_table_func_rights -taulusta viittaukset, joiden funktio puuttuu tai on disabloitu...")
	...
	log.Printf("Poistettu %d riviä restricted.auth_group_table_func_rights -taulusta (funktio puuttui tai oli disabloitu).", rowsAffected)
	return nil
}
...
func getPermissions(w http.ResponseWriter, _ *http.Request) {
	query := `
		...
		FROM restricted.auth_group_table_func_rights
	`
	rows, err := backend.DbRestricted.Query(query)
	if err != nil {
...
func createPermissions(w http.ResponseWriter, r *http.Request) {
	...
	if tableName == "" {
        ...
        `
		res, err := backend.DbRestricted.Exec(deleteQuery, schemaName)
		if err != nil {
		...
		countQuery := `
            SELECT COUNT(*) FROM restricted.auth_group_table_func_rights
            WHERE target_schema_name = $1 AND target_table_name = $2
        `
		var count int
		if err := backend.DbRestricted.QueryRow(countQuery, schemaName, tableName).Scan(&count); err != nil {
		...
		if count > 0 {
			delQuery := `
                ...
                WHERE target_schema_name = $1 AND target_table_name = $2
            `
			res, err := backend.DbRestricted.Exec(delQuery, schemaName, tableName)
			if err != nil {
...
func insertPermission(p Permission) error {
	query := `
        INSERT INTO restricted.auth_group_table_func_rights
        ...
        DO NOTHING
    `
	_, err := backend.DbRestricted.Exec(query, p.AuthUserGroupID, p.FunctionID, p.TargetSchemaName, p.TargetTableName)
	if err != nil {
