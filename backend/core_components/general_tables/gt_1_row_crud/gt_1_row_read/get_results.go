...
func buildWhereClause(
	...
	for param, values := range queryParams {
		...
		// -------- 1) *_from / *_to  (>= / <=) ----------------------------
		if strings.HasSuffix(plainParamName, "_from") || strings.HasSuffix(plainParamName, "_to") {
			var op string
			var colBase string

			if strings.HasSuffix(plainParamName, "_from") {
				op = ">="
				colBase = strings.TrimSuffix(plainParamName, "_from")
			} else {
				op = "<="
				colBase = strings.TrimSuffix(plainParamName, "_to")
			}
		...
		if expr, ok := columnExpressions[plainParamName]; ok {
			targetColumn = expr
		} else if _, ok := columnsByName[plainParamName]; ok {
			targetColumn = fmt.Sprintf("%s.%s",
				pq.QuoteIdentifier(tableName),
				pq.QuoteIdentifier(plainParamName),
			)
		} else {
		...
		cond, condArgs, nextIdx := buildConditionForTokens(targetColumn, tokens, argIdx)
		if cond != "" {
