...
// 		}
// 		if !allowedColumnsMap[cs.ColumnName] {
// 			// Sarakkeelle ei ole SELECT-oikeutta => jätetään pois
// 			continue
// 		}
// 		for uid, colInfo := range columnsMap {
// 			if colInfo.ColumnName == cs.ColumnName {
...
// 	selectColumns, joinClauses, columnExpressions, err := buildJoins(
// 		table_name,
// 		columnsMap,
// 		visibleColUids,
// 	)
...
// // buildJoins rakentaa SELECT-osan ja JOIN-lauseet ulkoavainten (FK) perusteella.
// //
// // Huom! Ei ota db:tä parametrina, jotta se sopii nykyiseen kutsutapaan:
// //
// //	buildJoins(tableName, columnsMap, columnUids)
// //
// // Jos haluat dynaamisen DB-käytön, utils.GetForeignKeysForTable()
// // tulee itse hoitaa (mahdollisesti) session/roolin mukaisesti.
// func buildJoins(
// 	tableName string,
...
// 	// Tässä oletamme, että utils.GetForeignKeysForTable(tableName)
// 	// käyttää joko globaalia kantayhteyttä (esim. backend.Db),
// 	// tai osaa itse valita rooliyhteyden. Tällä hetkellä koodisi
// 	// kutsuu suoraan: utils.GetForeignKeysForTable(tableName).
...
// 		// Tarkistetaan, onko colName foreignKeys-mappimme avain
...
// 			fullyQualifiedColumnName := fmt.Sprintf("%s.%s",
...
// 		} else {
// 			// Ei vierasavain tai nimisaraketta
...
// // buildWhereClause kerää URL-parametrit => ehdot (ILIKE, jne.).
// // Ei tee DB-kyselyitä, joten db-parametri ei ole tarpeen.
// func buildWhereClause(
// 	queryParams url.Values,
// 	tableName string,
// 	columnsByName map[string]models.ColumnInfo,
...
// 	for param, values := range queryParams {
// 		if param == "list" || param == "sort_column" || param == "sort_order" {
// 			continue
// 		}
// 		if len(values) == 0 {
// 			continue
...
// // buildConditionForTokens rakentaa ehdon annetuista hakutokeneista.
// func buildConditionForTokens(
// 	columnName string,
// 	tokens []Token,
// 	argIndex int,
...
// // 	"easelect/backend/core_components/general_tables/utils"
// // 	e_sessions "easelect/backend/core_components/sessions"
// // 	// HUOM: Tarvitset importin, jossa on auth.RoleDbMapping
// // 	// jos RoleDbMapping asuu esim. paketissa "easelect/backend/core_components/auth"
// // 	// lisää se import, esim:
// // 	// "easelect/backend/core_components/auth"
// // 	// ja käytä auth.RoleDbMapping[userRole]
// // )
...
// // 	// Tässä kartassa voi olla esim. "admin" -> backend.DbAdmin, "basic" -> backend.DbBasic jne.
// // 	// Jos käytät auth.RoleDbMapping, hae se sieltä:
// // 	// currentDb, found := auth.RoleDbMapping[userRole]
// // 	// if !found {
// // 	//    currentDb = auth.RoleDbMapping["guest"]
// // 	// }
// // 	// Alla vain esimerkkikartta. Mukauta omaan käyttöön.
// // 	roleDbMapping := map[string]*sql.DB{
// // 		"admin": backend.DbAdmin,
// // 		"basic": backend.DbBasic,
// // 		"guest": backend.DbGuest,
...
// // 	// 2. Haetaan results_per_load ja offset käyttäen currentDb
// // 	var results_per_load_str string
// // 	err = currentDb.QueryRow("SELECT int_value FROM system_config WHERE key = 'results_load_amount'").
// // 		Scan(&results_per_load_str)
// // 	if err != nil {
// // 		log.Printf("\033[31mvirhe: %s\033[0m\n", err.Error())
// // 		http.Error(response_writer, "virhe konfiguraatiota haettaessa", http.StatusInternalServerError)
...
// // 	// 3. Haetaan käyttäjän sarakeasetukset. Käytämme currentDb
// // 	userColumnSettings, err := ensureAndFetchUserColumnSettings(userID, table_name, currentDb)
// // 	if err != nil {
// // 		log.Printf("\033[31mvirhe: %s\033[0m\n", err.Error())
// // 		http.Error(response_writer, "virhe sarakeasetuksissa", http.StatusInternalServerError)
...
// // 	// 4. Haetaan muun datan osalta saraketietoja, tietotyyppejä ym. currentDb
// // 	column_data_types, err := getColumnDataTypesWithFK(table_name, currentDb)
// // 	if err != nil {
// // 		log.Printf("\033[31mvirhe: %s\033[0m\n", err.Error())
// // 		http.Error(response_writer, "virhe sarakkeiden tietotyyppien haussa", http.StatusInternalServerError)
...
// // 	// Haetaan sarakkeet (globaalisti nämä hakevat taulutiedot, voi olla ok)
// // 	columnsMap, err := gt_2_column_read.GetColumnsMapForTable(table_name)
// // 	if err != nil {
// // 		log.Printf("\033[31mvirhe: %s\033[0m\n", err.Error())
// // 		http.Error(response_writer, "virhe sarakkeiden tietojen haussa", http.StatusInternalServerError)
...
// // 	// Rakennetaan SELECT- ja JOIN-osat näkyville sarakkeille (käyttäen currentDb)
// // 	selectColumns, joinClauses, columnExpressions, err := buildJoins(
// // 		table_name,
// // 		columnsMap,
// // 		visibleColUids, // huom: vain kolme argumenttia
// // 	)
// // 	if err != nil {
// // 		log.Printf("\033[31mvirhe: %s\033[0m\n", err.Error())
// // 		http.Error(response_writer, "virhe JOIN-liitosten rakentamisessa", http.StatusInternalServerError)
...
// // 	// Rakennetaan WHERE- ja ORDER BY -ehdot (ei Query-kutsua => ei tarvita currentDb param.)
// // 	where_clause, query_args, err := buildWhereClause(
// // 		request.URL.Query(),
// // 		table_name,
// // 		buildColumnsByName(columnsMap),
