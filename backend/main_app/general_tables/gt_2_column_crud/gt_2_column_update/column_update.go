...
import (
	"database/sql"
	gt_2_column_crud "easelect/backend/main_app/general_tables/gt_2_column_crud"
...
// 		// Hae saraketiedot ilman attnum:ia
// 		columnsQuery := fmt.Sprintf(`
// 			SELECT a.attname,
// 				pg_catalog.format_type(a.atttypid, a.atttypmod) AS data_type
// 			FROM pg_attribute a
...
// 		// Talletetaan sarakkeen nimi -> data_type
// 		type ColumnSmallInfo struct {
// 			DataType string
// 		}
// 		existingColumns := make(map[string]ColumnSmallInfo)
...
// 		for colName, col := range existingColumns {
// 			if metaInfo, exists := existingMetadata[colName]; exists {
// 				// Sarake on jo olemassa, päivitetään vain column_name ja data_type.
// 				updateQuery := `
//                     UPDATE system_column_details
//                     SET column_name = $1,
//                         data_type   = $2
...
// 				insertQuery := `
//                     INSERT INTO system_column_details
//                         (table_uid, column_name, data_type)
//                     VALUES ($1, $2, $3)
//                 `
// 				_, err = backend.Db.Exec(
// 					insertQuery,
...
// 		// Hae olemassa olevat system_column_details-rivit käyttäen uutta kenttänimeä co_number
// 		metadataQuery := `
//             SELECT co_number, column_name, column_uid, data_type
//             FROM system_column_details
//             WHERE table_uid = $1
//         `
// 		metaRows, err := backend.Db.Query(metadataQuery, table.TableUID)
...
// 		// Talletetaan co_number -> (column_uid, data_type)
// 		type ExistingMeta struct {
// 			ColumnUID int
...
// 			if err := metaRows.Scan(&orderNumber, &columnName, &columnID, &dataType); err != nil {
// 				log.Printf("error scanning metadata for table %s: %v",
// 					table.TableName, err)
// 				continue
// 			}
// 			existingMetadata[orderNumber] = ExistingMeta{
// 				ColumnUID: columnID,
// 				DataType:  nilIfEmpty(dataType),
// 			}
// 		}
...
// 			} else {
// 				// Uusi sarake, lisätään se.
// 				// co_number (aiemmin attnum) asetetaan kerran INSERTissä.
// 				insertQuery := `
//                     INSERT INTO system_column_details
//                         (table_uid, co_number, column_name, data_type)
//                     VALUES ($1, $2, $3, $4)
//                 `
// 				_, err = backend.Db.Exec(
// 					insertQuery,
...
// 		// Poista sarakkeet, joita ei enää ole
// 		for orderNumber := range existingMetadata {
// 			if _, exists := existingColumns[orderNumber]; !exists {
// 				deleteQuery := `
//                     DELETE FROM system_column_details
//                     WHERE table_uid = $1 AND co_number = $2
//                 `
// 				_, err = backend.Db.Exec(deleteQuery, table.TableUID, orderNumber)
// 				if err != nil {
// 					log.Printf("error deleting from system_column_details for table %s, co_number %d: %v",
// 						table.TableName, orderNumber, err)
// 					continue
// 				}
// 			}
// 		}
...
// import (
// 	"database/sql"
// 	"easelect/backend/main_app/general_tables/gt_2_column_crud"
// 	backend "easelect/backend/main_app"
// 	"fmt"
// 	"log"
// 	"strings"
