...
function createHeaderCell(column, table_name) {
    ...
    // Ei enää tarvetta "pointer" -kursoriin koko solulle
    th.style.cursor = 'default';

    const columnSpan = document.createElement('span');
    columnSpan.textContent = column;
    // Jos haluat myös columnSpanille eri kursorin:
    // columnSpan.style.cursor = 'text';

    const sortIndicator = document.createElement('span');
    sortIndicator.classList.add('float_right');

    // Laitetaan nuolelle hiiren kursoriksi pointer
    ...
    // Lisätään klikkauskuuntelija vain sortIndicatorille
    sortIndicator.addEventListener('click', (event) => {
        // estetään klikkauskuplan nouseminen th-elementtiin (varmuuden vuoksi)
...
function createFilterCell(column, table_name) {
    ...
    //piilota haku väliaikaisesti lisäämällä "display: none":
...
export function createDataCell(item, column, columns, rowIndex, colIndex) {
    ...
    if (columns.includes(foreignKeyColumn)) {
        ...
        value = item[column];
        td.title = (item[`${column}_name`] === null || item[`${column}_name`] === undefined) ? 'Tuntematon' : item[`${column}_name`];
        displayValue = formatValue(value);
        td.textContent = displayValue;
