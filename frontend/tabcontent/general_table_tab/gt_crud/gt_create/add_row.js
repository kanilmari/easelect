...
import { fetchTableData } from '../gt_read/endpoint_data_fetcher.js';
...
export async function open_add_row_modal(table_name) {
    ...
    const exclude_columns = ['id', 'created', 'updated', 'openai_embedding'];
    ...
    for (const column of columns) {
        ...
        if (column.foreign_table_name) {
            // Tämä on vierasavainsarake; luodaan dropdown
            const select = document.createElement('select');
            select.id = `${table_name}-${column.column_name}-input`;
            select.name = column.column_name;
            select.required = column.is_nullable.toLowerCase() === 'no';
            ...
            // Hae vaihtoehdot viitatusta taulusta
            console.log('opening fetchReferencedData for', column.foreign_table_name);
            ...
            options.forEach(option => {
                ...
                const pk_column = Object.keys(option).find(key => key !== 'display');
                opt.value = option[pk_column];
                opt.textContent = option['display'];
                select.appendChild(opt);
            });

            if (modal_form_state[column.column_name]) {
                select.value = modal_form_state[column.column_name];
            }
            ...
            // Vertaillaan sarakkeen tietotyyppiä
            ...
            if (
                ...
                // Jos käytetään auto-resize-läsnä olevaa koodia, varmistetaan heti aluksi:
                textarea.style.height = 'auto';
                textarea.value = modal_form_state[column.column_name] || '';
                textarea.style.height = textarea.scrollHeight + 'px';

                // (tai vastaava) “keinotekoinen” tapahtuman laukaisu:
