...
export async function render_tree(data, config) {
    ...
    // Säilytetään käyttäjän muokattavissa olevat asetukset global_configissa.
    const global_config = {
    ...
    // ------------------------------------------------------------------------
    // LocalStorage-pohjainen logiikka avattujen solmujen tallentamiseen
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // createNode: luo yhden puun solmun
    // Tässä on muutos: kansion nimen ja lapsilaskurin elementit erotetaan,
    // jolloin lang.js pystyy päivittämään vain kansion nimen ilman, että node-count häviää.
    // ------------------------------------------------------------------------
    function createNode(nodeData, level = 0) {
        ...
        node_element.setAttribute('data-is-folder',
            Array.isArray(nodeData.children) && nodeData.children.length > 0 ? 'true' : 'false'
        );
        ...
        // Apu: data-lang-key attribuutin asettaminen
        ...
        if (render_mode === 'checkbox' && should_include_checkbox) {
            ...
            checkbox.addEventListener('change', (e) => {
                ...
                // Käytetään uutta apufunktiota tässä
        ...
        // LUODAAN KANSIOIDEN JA LEHTIEN NIMET: erotetaan nimelle varattu elementti ja lapsilaskurille varattu elementti
        if (!(render_mode === 'button' && is_leaf)) {
            const label_container = document.createElement('span');
            label_container.style.userSelect = 'none';

            // Tämä elementti sisältää vain nimen, ja siihen asetetaan data-lang-key
            ...
            // Jos halutaan näyttää lapsien lukumäärä ja kyseessä on kansio, lisätään laskuri erikseen.
