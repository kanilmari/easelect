...
export function appendDataToCardView(card_container, columns, data, table_name) {
    ...
    data.forEach(item => {
        ...
        columns.forEach((column) => {
            ...
            const showValueOnCard = data_types[column]?.show_value_on_card === true;
            if (!showValueOnCard) {
    ...
    for (const row_item of data) {
        ...
        if (details_entries.length > 0) {
            ...
            for (const detailObj of details_entries) {
                ...
                // Luodaan ensin avain–arvo -elementti (jotta label näkyy, jos halutaan).
                const wrapper = createKeyValueElement(
                    detailObj.label,
                    '', // teksti lisätään alla
                    detailObj.column,
                ...
                // Jos details_link, luodaan suoraan linkkielementti
                if (detailObj.isLink) {
                    const linkElement = document.createElement('a');
                    linkElement.href = detailObj.rawValue.trim();
                    linkElement.target = '_blank';
                    linkElement.textContent = detailObj.rawValue.trim();
                    valueDiv.appendChild(linkElement);
                } else {
                    // Muussa tapauksessa tarkistamme pituuden
...
// /**
//  * Asynkroninen create_card_view: rakentaa kortit data-taulukosta,
//  * tukee roolit: header, image, description, details, keywords, username,
//  * details_link (uusi) sekä kieliavain "lang-key" (tai "lang_key").
//  *
...
//                 // details
//                 if (/^details(\d+)?$/.test(singleRole) && val_str.trim()) {
//                     // Tarkistetaan myös, onko show_key_on_card true
...
// //                         hasLangKey,
// //                         column,
// //                         isLink: false // oletus: tavallinen details
// //                     });
// //                     continue;
// //                 }
...
// //                         hasLangKey,
// //                         column,
// //                         isLink: true // merkitään linkiksi
// //                     });
// //                     continue;
// //                 }
