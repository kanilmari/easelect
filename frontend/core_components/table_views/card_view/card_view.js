    ...
    if (table_name.endsWith("locations")) {
        ...
        if (hasAllAddressCols) {
            ...
            if (embedSrc) {
                ...
                iframe.loading = "lazy";
                iframe.referrerPolicy = "no-referrer"; // kovempi referrer
...
/* ----------------------------------------------------------- */
/*  ►  A P U F U N K T I O  ◄                                  */
/*  Jos rivillä on osoitekentät --> Google-karttalinkki.       */
/* ----------------------------------------------------------- */
...
function generateGoogleMapsEmbedSrcFromRow(rowItem) {
    ...
    /* Ei vaadi API-avainta: “classic” embed + query + output=embed. */
    return (
