...
document.addEventListener('DOMContentLoaded', function() {
  ...
  radioInputs.forEach(function(radio) {
    radio.addEventListener('change', function() {
      localStorage.setItem('chosen_language', radio.value);
  ...
  let savedLang = localStorage.getItem('chosen_language') || navigator.language.split('-')[0];
...
function setDefaultMenuLanguage() {
  ...
  let savedLang = localStorage.getItem('chosen_language') || defaultLang;
...
export function updateMenuLanguageDisplay() {
  let chosen_language = document.querySelector('.floating-language-panel input[type="radio"]:checked')?.value;
  ...
  if (buttonElement) {
    ...
    let shortLangCode = getShortLanguageCode(chosen_language) || 'Select Menu Language';
