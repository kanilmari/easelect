...
import { open_chat_window, open_file_struct_ud_window } from './common_actions/builder_chat/builder_chat_upload.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 4. ENNEN nämä aukesivat automaattisesti, mutta nyt kommentoidaan pois:
    // open_chat_window();
    // open_file_struct_ud_window();

    // Nyt chat- ja tiedostonsiirtoikkunat aukeavat vasta, kun klikkaat navigaation “▶ Chat” -nappia.
});
...
// // Pääohjelma, joka lataa taulut ja custom-näkymät sekä hallitsee navigointia.
...
// // ---> Nämä importit, jos haluat avata draggable-ikkunoita:
// // import { open_file_struct_ud_window } from './common_actions/windows/windows.js';
...
//     // 4. Avataan koodi-chat-ikkuna (draggable) ja tiedostonsiirtoikkuna
//     //    Voit poistaa tai kommentoida pois, jos haluat ne vain nappipainalluksella.
//     open_chat_window();
//     open_file_struct_ud_window();
// });
...
//             throw new Error(`error updating OID values: ${response.statusText}`);
//         }
//         // console.log('OID values and table names updated.');
//     } catch (error) {
//         console.error('error updating OID values and table names:', error);
//     }
// }

// // Määritellään custom_views, mm. chat-näkymä “▶ Chat”
// const custom_views = [
//     {
//         name: '▶ Oikeuksien hallinta',
//         loadFunction: load_rights_management,
...
// // Ladataan sivulla näkyvä lista tauluista (vas. reunan navigaatio tms.)
// async function load_tables() {
//     try {
//         const response = await fetch('/api/tables');
//         if (!response.ok) {
...
// /**
//  * Palauttaa luku-funktion ja container-id:n
//  * sen mukaan, onko kysessä custom-näkymä vai tavallinen taulu.
//  */
// export function get_load_info(name, custom_views) {
//     const custom_view = custom_views.find(view => view.name === name);
//     if (custom_view) {
...
//         // Säilötään localStorageen, jos haluat käyttää myöhemmin
//         localStorage.setItem(`${table_name}_columns`, JSON.stringify(response_columns));
//         localStorage.setItem(`${table_name}_dataTypes`, JSON.stringify(data_types));
...
// // Lisälogiikkaa (taulukon otsikoiden leveyden säätö tms.)
// document.addEventListener("DOMContentLoaded", function() {
//     const table_headers = document.querySelectorAll("#auth_user_groups_table th");
//     table_headers.forEach(function(table_header) {
//         let existing_resize_handle = table_header.querySelector(".resize-handle");
...
// //         const grouped_tables = result.tables; // array
// //         create_navigation_buttons(custom_views);
