...
export async function render_tree(data, config) {
    ...
    stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="12 6, 18 12, 12 18"/></svg>`;
    ...
    function createNode(nodeData, level = 0) {
        ...
        if (has_children) {
            toggle_element.innerHTML = svgToggle;
            ...
            toggle_element.innerHTML = `<svg class="toggle-icon" viewBox="0 0 24 24" 
                fill="none" stroke="none" stroke-width="0"></svg>`;
        }
        ...
        if (global_config.use_icons) {
            ...
            icon_element.innerHTML = svgIcon;
        ...
        if (has_children) {
            ...
            // Ei käytetä animaatiota: piilotetaan kontti heti
    ...
    function openChildren(nodeElement) {
        ...
        // Näytetään kontti välittömästi ilman animaatiota
        container.style.display = 'block';
        container.style.transition = ''; // Poistetaan mahdolliset transition-efektit
        // Käytetään maxHeight-arvoa tilan tunnistamiseen (avattuna = 'none')
        container.style.maxHeight = 'none';

        // Päivitetään esivanhempien koot (ei toistuvaa päivitystä)
        update_ancestor_container_heights(nodeElement);

        // LocalStorage: merkitään nodeElement avoimeksi
    ...
    function closeChildren(nodeElement) {
        ...
        // Piilotetaan kontti välittömästi ilman animaatiota
        ...
        // Päivitetään myös vanhempien kontit
        update_ancestor_container_heights(nodeElement);
    }
    ...
    function toggleChildrenVisibility(container) {
        ...
        // Tarkistetaan display-tila avauksen/piilotuksen päätöksenteossa
        ...
        nodes.forEach(node => {
            ...
            if (text.includes(searchTerm.toLowerCase())) {
                ...
                while (parent) {
                    ...
                    // Tarkistetaan, onko kontti piilotettu display-arvon perusteella
    ...
    async function render(data) {
        ...
        setTimeout(() => {
            // Avataan ne nodet, jotka aluksi on määritelty avoimiksi
            ...
            storedExpandedNodes.forEach(nodeId => {
                ...
                if (!nodeElem) return;
                // Tarkistetaan, että kaikki nodeElem:n esivanhemmat ovat myös tallennettuina avoimina
...
// // // Apufunktio, joka laskee containerin korkeuden ja päivittää maxHeightin
// // function recalc_container_height(container) {
// //     if (!container) return;
// //     // Asetetaan ensin max-height nollaksi ja pakotetaan reflow
// //     container.style.maxHeight = '0px';
// //     container.offsetHeight; // pakotetaan reflow
// //     const total_height = container.scrollHeight;
// //     // Asetetaan uusi max-height, jolloin animaatio käynnistyy
// //     container.style.maxHeight = total_height + 'px';
// // }

// // Apufunktio, joka päivittää kaikkien vanhempien (ancestor) konttien maxHeightit rekursiivisesti
// function update_ancestor_container_heights(node_element) {
//     let parent_node = node_element.parentElement?.closest('.node');
//     while (parent_node) {
//         const parent_container = parent_node.querySelector(':scope > .children');
//         if (
//             parent_container 
//             && parent_container.style.maxHeight !== '0px'
//             && parent_container.style.maxHeight !== ''
//         ) {
//             animate_container_height(parent_container);
//         }
//         parent_node = parent_node.parentElement?.closest('.node');
//     }
// }

// function animate_container_height(container) {
//     // Jos se on jo piilotettu, ei tehdä mitään
//     if (container.style.maxHeight === '0px') return;

//     // Jos container on jo 'none' (täysin auki), 
//     // vaihdetaan pikselilukemaksi, jotta animointi onnistuu
//     if (container.style.maxHeight === 'none') {
//         container.style.maxHeight = container.scrollHeight + 'px';
//     }
    
//     container.offsetHeight; // pakotetaan selain lukemaan edellinen arvo

//     // Nyt mitataan uudelleen, paljonko tilaa tarvitaan
//     const new_height = container.scrollHeight;
//     container.style.transition = 'max-height 0s ease-in-out';
//     container.style.maxHeight = new_height + 'px';

//     // Kun animaatio on valmis, asetetaan taas none
...
//                 const current_node = e.target.closest('.node');
//                 const possible_children_container = current_node.querySelector('.children');
//                 const is_folder = !!possible_children_container;
...
//             children_container.className = 'children';
//             children_container.style.overflow = 'hidden';
//             children_container.style.maxHeight = '0';
//             children_container.style.transition = 'max-height 0s ease-in-out';
...
//     // Uusi openChildren, jossa on requestAnimationFrame-loop, 
//     // jotta vanhemman korkeus päivittyy reaaliaikaisesti
//     function openChildren(nodeElement) {
//         const container = nodeElement.querySelector('.children');
//         if (!container) return;

//         // Varmistetaan, että container näkyy
//         container.style.display = 'block';

//         // Jos se on jo täysin auki (maxHeight === 'none'), päivitetään esivanhemmat ja lopetetaan
//         if (container.style.maxHeight === 'none') {
//             update_ancestor_container_heights(nodeElement);
//             return;
//         }

//         // Aloitus: asetetaan maxHeight = '0', pakotetaan reflow, sitten asetetaan lopullinen korkeus
//         container.style.maxHeight = '0';
//         container.offsetHeight; // pakotetaan reflow

//         const full_height = container.scrollHeight;
//         container.style.transition = 'max-height 0s ease-in-out';
//         container.style.maxHeight = full_height + 'px';

//         // Käynnistetään jatkuva päivitys, jotta myös vanhemmat kasvavat samassa tahdissa
//         let update_loop_id = requestAnimationFrame(function updateLoop() {
//             update_ancestor_container_heights(nodeElement);
//             update_loop_id = requestAnimationFrame(updateLoop);
//         });

//         // Kun animaatio on valmis, pysäytetään loop ja annetaan laatikon jäädä auki (maxHeight='none')
//         const onTransitionEnd = (evt) => {
//             if (evt.propertyName === 'max-height') {
//                 cancelAnimationFrame(update_loop_id);
//                 container.style.maxHeight = 'none';
//                 container.removeEventListener('transitionend', onTransitionEnd);
//             }
//         };
...
//         // Päivitetään myös varmuuden vuoksi vielä vanhemman koko heti kerran
//         update_ancestor_container_heights(nodeElement);
//     }
...
//         // Jos se on jo suljettu, ei tehdä mitään
//         if (container.style.maxHeight === '0px') {
//             return;
//         }

//         // Jos maxHeight on 'none', asetetaan pikseliarvo, jotta sulkeutuminen animoituu
//         if (container.style.maxHeight === 'none') {
//             container.style.maxHeight = container.scrollHeight + 'px';
//         }
...
//         // Päivitetään myös vanhempien kontit, jotta ne pienenevät lapsen sulkeutuessa
//         update_ancestor_container_heights(nodeElement);
//     }
