...
// UUSI KOODI: huomaa, miten splitataan plus-merkillä, etsitään käännösavaimen varsinainen osa (baseKey),
// ja jos plus-merkin jälkeen on lisätieto, korvataan se käännöksestä löytyvästä $table_name-merkinnästä.
function translateElement(one_element, translation_data, chosen_language, missing_keys) {
    ...
    // Jos avain sisältää plus-merkin, pilkotaan se (esim. "manage_table+dev_dating_profiles")
    ...
    var variable_part = splitted_key.length > 1 ? splitted_key[1] : null;

    // Haetaan varsinainen käännös base_keyllä
    var one_translation = translation_data[base_key];

    // Jos käännös löytyy, korvataan tarvittaessa $table_name, jos muuttuja on annettu
    if (one_translation) {
        if (variable_part) {
            // Korvataan mahdollinen $table_name -placeholder
        ...
        if (one_element.tagName.toLowerCase() === 'input') {
            if (one_element.hasAttribute('placeholder')) {
                ...
                if (debug) console.log("input element with placeholder: " + one_translation);
            } else if (one_element.getAttribute('type') === 'submit') {
                one_element.setAttribute('value', one_translation);
                if (debug) console.log("input element of type submit: " + one_translation);
            }
        } else if (
            ...
            if (debug) console.log("option or element with child span/i: " + one_translation);
        } else {
            one_element.innerHTML = one_translation;
            if (debug) console.log("general element: " + one_translation);
        }
    } else {
        // Jos käännöstä ei löydy, lisätään (jos seurataan puuttuvia avaimia)
        if (missing_keys) {
...
// Käynnistä käännös heti sivun latautuessa
