...
function filterColumns(columns_info) {
  ...
  return columns_info.filter(col => {
      ...
      // Poistettu allow_form_insertion_on_source -logiikka

      // Poistettu input_specs-tarkistus
      // (aiemmin saattoi piilottaa kentän, jos specs.inputType === "none")
...
function buildForeignKeyField(form, table_name, column) {
  ...
  fetchReferencedData(column.foreign_table_name)
    .then(options => {
      ...
      // (Jos käytössä .__dropdown.setOptions(...), käytä sitä. Tässä demossa ohitettu.)
    })
    .catch(err => {
