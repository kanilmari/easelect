...
async function generate_permissions_form(table_based_permissions_container) {
    ...
    const [
        ...
        fetch_general_table_functions(),  // general_table_related: true
        fetch_tableless_functions(),     // general_table_related: false
        fetch_user_groups()
    ]);
    ...
    function createFunctionRows(funcList, targetArr) {
        funcList.forEach((function_item) => {
            ...
            cleaned_name = cleaned_name.replace(/\./, ':<br>');
    ...
    table_based_permissions_container.appendChild(container_with_mode_buttons);
...
async function fetch_general_table_functions() {
    ...
    console.log('general_table_related=true rivit', result);
...
async function fetch_tableless_functions() {
    const result = await fetchTableData({
        table_name: 'functions',
        filters: {
            general_table_related: 'false'
        }
    });
    console.log('general_table_related=false (tauluttomat funktiot)', result);
    return result.data.map(fn => ({
        id: fn.id,
        name: fn.name
    }));
...
// Käsittelee vain taululliset ryhmien funktio-oikeudet:
...
// async function generate_permissions_form(table_based_permissions_container) {
//     const [function_list, user_group_list] = await Promise.all([
