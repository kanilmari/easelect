...
export function createViewSelectorButtons(table_name, current_view) {
  ...
  // HUOM: Tämä koodi kutsuu `load_table(table_name)`-funktiota, jonka oletan
  // olevan jossain muussa tiedostossa. Varmista, että se on globaali
  // tai importtaa se esim. näin (jos sijaitsee esim. "table_loader.js":ssä):
  //
  // import { load_table } from '../path/to/table_loader.js';
  //
  // tai jos sinulla on jokin muu tapa päivittää näkymä (switchView tms.),
  // korvaa tuo `load_table(table_name);` -kutsu haluamallasi.
